...
namespace SaralESuvidha.Models
{
    public class RetailUser : IDisposable
    {
        ...
        public RetailUserBalanceResponse GetUPPCLBalance(string retailUserId="")
        {
            RetailUserBalanceResponse rubr = new RetailUserBalanceResponse();
            try
            {
                using (var con = new SqlConnection(StaticData.conString))
                {
                    var queryParameters = new DynamicParameters();
                    queryParameters.Add("@RetailUserId", retailUserId);
                    rubr = con.QuerySingle<RetailUserBalanceResponse>("usp_GetUserUPPCLBalance", queryParameters, commandType: System.Data.CommandType.StoredProcedure);
                }
            }
            catch (Exception ex)
            {
        ...
        public OperationResponse FundRequest(string userId, int userOrderNo, decimal amt, string remarks, string requestIp, string requestMachine, string requestGeoCode, string requestNumber, string requestMessage, DateTime requestTime, int requestSource)
        {
            OperationResponse result = new OperationResponse();
            try
            {
                using (var con = new SqlConnection(StaticData.conString))
                {
                    ...
                    {
                        var queryParameters = new DynamicParameters();
                        queryParameters.Add("@OrderNo", userOrderNo);
                        queryParameters.Add("@UserId", userId);
                        queryParameters.Add("@Amount", amt);
                        queryParameters.Add("@TranType", 18);
                        queryParameters.Add("@RequestIp", requestIp);
                        queryParameters.Add("@RequestMachine", requestMachine);
                        queryParameters.Add("@RequestGeoCode", requestGeoCode);
                        queryParameters.Add("@RequestNumber", requestNumber);
                        ...
                        result = con.QuerySingle<OperationResponse>("usp_RetailClientFundRequest", queryParameters, commandType: System.Data.CommandType.StoredProcedure);
                    }
                    catch (Exception e)
                    {
                        result.OperationMessage = "Errors: Ex:  " + e.Message;
